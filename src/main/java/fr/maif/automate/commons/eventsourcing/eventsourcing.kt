...
abstract class Store<S, C: Command, E: Event>(private val name: String, private val initialState: () -> S, private val eventStore: EventStore) {
    ...
    protected abstract fun applyEventToState(current: S, event: EventEnvelope): S

    protected abstract fun applyCommand(state: S, command: C): Single<Either<Error, E>>

    fun onCommand(command: C): Single<Either<Error, E>> =
            state().flatMap { s -> applyCommand(s, command) }
