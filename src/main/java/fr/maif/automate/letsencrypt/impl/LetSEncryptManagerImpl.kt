...
class LetSEncryptManagerImpl(
    ...
    override fun orderCertificate(
        domain: String,
        subdomain: Option<String>,
        isWildCard: Boolean
    ): Single<Either<Error, LetSEncryptCertificate>> {
        ...
        val obsK: ObservableK<Either<Error, LetSEncryptCertificate>> =
            EitherT.monadError<ForObservableK, Error>(ObservableK.monad()).binding {

                val acc =
                    letSEncryptUser.getOrCreateAccount(config.accountId).obsKRightT<Error, LetSEncryptAccount>().bind()
                ...
                val order =
                    orderLetsEncryptCertificate(account, effectiveDomain, isWildCard).obsKRightT<Error, Order>().bind()
        ...
        return obsK
            ...
            .doOnSuccess { _ ->
                removeLetsEncrypotDnsRecords(domain, subdomain)
                    .map {
                        it.fold({ l ->
                            throw RuntimeException(l.message)
                        }, { it })
                    }
                    .retry(3) { e -> true }
                    .subscribe({ s ->
    ...
    private fun doChallenges(
        order: Order,
        rootDomain: String,
        domain: String,
        subdomain: Option<String>
    ): Single<Either<Error, List<Status>>> {
    ...
    private fun createLetSEncryptDnsRecord(
        rootDomain: String,
        subdomain: Option<String>,
        challenge: Dns01Challenge
    ): Single<Either<Error, Record>> {
    ...
    private fun removeLetsEncrypotDnsRecords(
        rootDomain: String,
        subdomain: Option<String>
    ...
    private fun pollDns(domain: String, record: Record): Single<Either<Error, Pair<Record, List<String>>>> =
        Observable
            ...
            .doOnSuccess { event ->
                event.fold({ err ->
                    LOGGER.error("Error polling dns record {}: {}", record, err)
                }, { _ ->
                    LOGGER.error("Success polling dns {}", record)
                })
            }
        ...
        if (status == Status.VALID) {
            ...
            if (challenge.status == Status.VALID) {
                ...
                return Observable
                    ...
                    .doOnSuccess { event ->
                        event.fold({ err ->
                            LOGGER.error("Error authorizing order {}", err)
                        }, { _ ->
                            LOGGER.error("Order for domain {} authorized ", domain)
                        })
                    }
            }
        }
        ...
        Single.fromCallable {
            ...
            .flatMap { o ->
                Observable
                    ...
                    .single(Left(Error("Certificate not accepted for ${order.identifiers.map { it.domain }}")) as Either<Error, Order>)
            }
            .doOnSuccess { event ->
                event.fold({ err ->
                    LOGGER.error("Error ordering certificate {}", err)
                }, { _ ->
                    LOGGER.error("Ordering certificate for {} succed", order.identifiers.map { it.domain })
                })
            }
}
...
class LetSEncryptUserPostgres(private val pgClient: SQLClient) : LetSEncryptUser {
